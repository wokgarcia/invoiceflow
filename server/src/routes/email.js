...
function buildPdfBuffer(invoice, items, user) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(GRAY).fontSize(9).font('Helvetica').text('Subtotal', totalsX, y, { width: totalsW, align: 'right' });
    doc.fillColor(DARK).fontSize(9).font('Helvetica').text(fmt(invoice.subtotal, invoice.currency), totalsX, y, { width: totalsW + 50, align: 'right' });
    y += 15;
